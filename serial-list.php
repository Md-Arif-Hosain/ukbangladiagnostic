...
if ( isset( $_GET[ 'testname' ] ) ) {
    $filterDate = $_GET[ 'testname' ];
    $sql = "SELECT * FROM patientserial WHERE testname='$filterDate'";
    $result = $conn->query( $sql );
}
...
<h1>UK-Bangla Diagnostic Serial List</h1>
<!-- Filter Search Form -->
<form class = 'filter' action = 'serial-list.php' method ='get'>
<span> Search Your Test name:  </span>
<select type="testname" name="testname">
...
if ( $result->num_rows > 0 ) {
    ...
    //     $total="$s $t";
        ...
        <td scope = 'row'> <?php echo $row->testname ?>
        </td>
      <td scope = 'row'> <?php echo $row->phone ?>
        </td>
        ...
        <a href = "view-serial.php?id=<?php echo $row->id; ?>"><button>View</button></a>
